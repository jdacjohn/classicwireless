...
sub do_shop_now {
  ...
  my $query = 'select a.listname,a.sid,b.category,b.item_num,c.item_num from categories a, products b, specials c where a.sid = b.category and b.item_num = c.item_num ';
  ...
  my @sorted = sort { $temphash{$a} cmp $temphash{$b} } keys %temphash;
  my @categories = ();
  foreach my $key (@sorted) {
  &debug("Key = " . $key);
    my %row = ();
    %row->{'cat_sid'} = $key;
    %row->{'cat_name'} = $temphash{$key};
    push(@categories,\%row);
  }
  # Reserved for relatively recent versions of mysql
  #my @categories = ();
...
#    3.1.06 - jarnold
# 4. 03.20.2006 - Added qty field - jarnold
# 5. 03.23.3006 - Added code for inserting back to top links every 10 rows - jarnold
#------------------------------------------------------------------------------
sub do_default {
	my $query = 'SELECT sid,item_num,title,manuf,descr,price,phto_ext,qty FROM specials '.
  ...
  my $count = 1;
	while(my $row = $sth->fetchrow_hashref()) {
		...
		delete($row->{phto_ext});
    my $testString = $row->{descr};
    $testString =~ s#/# / #g;
    &debug("New description string is...." . $testString . "<br />");
		$row->{price} = sprintf('$%.2f',$row->{price});
    $row->{descr} = $testString;
    if (($count % 10) == 0) {
      $row->{back_to_top} = 1;
    }
		push(@specials,$row);
...
sub do_view_by_cat {
  ...
  my $query = 'select a.vendorid,a.sid,a.item_num,a.title,a.manuf,a.descr,a.price,a.phto_ext, a.qty,b.category from specials a, products b '.
